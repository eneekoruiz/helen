 ...
 */
export function printDoctorResults(checks: DoctorCheck[], project: ProjectInfo): void {
  ...
  logger.blank();
  console.log(
    `  ${pc.green(`${ok.length} passed`)}  ${pc.yellow(`${warnings.length} warnings`)}  ${pc.red(`${errors.length} errors`)}`,
  );
