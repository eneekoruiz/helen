...
const meta = {
    ...
    description: 'Sets up ESLint with TypeScript plugin, Prettier for auto-formatting, and a strict tsconfig. Adds lint, format, and typecheck scripts to package.json.',
    problemItSolves: 'Without consistent linting and formatting, codebases accumulate inconsistencies, bugs slip through, and PRs become harder to review.',
...
async function execute(ctx) {
    ...
    if (eslintResult === 'created' || eslintResult === 'overwritten')
        result.created.push('.eslintrc.json');
    else
        result.skipped.push('.eslintrc.json');
    // .prettierrc
    ...
    if (prettierResult === 'created' || prettierResult === 'overwritten')
        result.created.push('.prettierrc');
    else
        result.skipped.push('.prettierrc');
    // .prettierignore
...
`;
    ...
    if (ignoreResult === 'created' || ignoreResult === 'overwritten')
        result.created.push('.prettierignore');
    else
        result.skipped.push('.prettierignore');
...
export const qualityModule = { meta, execute };
